#include <array/mean_of_array_after_removing_some_elements.cpp>

TEST(删除某些元素后的数组均值, trimMean) {
  // 示例 1：
  ...
  vector<int> arr = {1, 2, 2, 2, 2, 2, 2, 2, 2, 2,
                     2, 2, 2, 2, 2, 2, 2, 2, 2, 3};
  EXPECT_DOUBLE_EQ(solution.trimMean(arr), 2.00000);

  // 示例 2：
  // 输入：arr = [6,2,7,5,1,2,0,3,10,2,5,0,5,5,0,8,7,6,8,0]
  // 输出：4.00000
  vector<int> arr2 = {6, 2, 7, 5, 1, 2, 0, 3, 10, 2,
                      5, 0, 5, 5, 0, 8, 7, 6, 8,  0};
  EXPECT_DOUBLE_EQ(solution.trimMean(arr2), 4.00000);

  // 示例 3：
  // 输入：arr =
  // [6,0,7,0,7,5,7,8,3,4,0,7,8,1,6,8,1,1,2,4,8,1,9,5,4,3,8,5,10,8,6,6,1,0,6,10,8,2,3,4]
  // 输出：4.77778
  vector<int> arr3 = {6,  0, 7, 0, 7, 5, 7, 8,  3, 4, 0, 7, 8, 1,
                      6,  8, 1, 1, 2, 4, 8, 1,  9, 5, 4, 3, 8, 5,
                      10, 8, 6, 6, 1, 0, 6, 10, 8, 2, 3, 4};
  double res = 4.77778;
  EXPECT_NEAR(solution.trimMean(arr3), res, 10e-5);
  ...
  // 输出：5.27778
  vector<int> arr4 = {9, 7, 8,  7, 7,  8, 4,  4,  6, 8, 8, 7, 6, 8,  8,
                      9, 2, 6,  0, 0,  1, 10, 8,  6, 3, 3, 5, 1, 10, 9,
                      0, 7, 10, 0, 10, 4, 1,  10, 6, 9, 3, 6, 0, 0,  2,
                      7, 0, 6,  7, 2,  9, 7,  7,  3, 0, 1, 6, 1, 10, 3};
  EXPECT_NEAR(solution.trimMean(arr4), 5.27778, 10e-5);
  ...
  // 输出：5.29167
  vector<int> arr5 = {4,  8, 4, 10, 0,  7,  1, 3, 7,  8,  8, 3,  4, 1,  6, 2,
  ...
  EXPECT_NEAR(solution.trimMean(arr5), 5.29167, 10e-5);
}
